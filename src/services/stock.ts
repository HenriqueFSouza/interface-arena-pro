...
class StockService {
  ...
  async updateUnitPrice(id: string, data: UpdateUnitPriceDTO) {
    const response = await api.put<StockItem>(`/stock/${id}/unit-price`, data);
    return response.data;
  }
  ...
  async getHistory(stockId: string) {
    const response = await api.get<StockHistoryEntry[]>(
      `/stock-history/${stockId}`
    );
    return response.data;
  }
}
